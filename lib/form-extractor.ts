...
export async function extractFormQuestions(
  url: string, 
  formId: string,
  addQuestionMutation: (args: { formId: string; question: string }) => Promise<any>
  ...
  try {
    ...
    if (hostname.includes('docs.google.com')) {
      ...
      return {
        success: false,
        error: "This URL is not from a supported form provider. We currently support Google Forms and Airtable.",
      };
    }
...
async function saveQuestionsToDatabase(
  ...
  for (const question of questions) {
    try {
      ...
      console.error(`Failed to save question to database: ${question.text}`, error);
    }
  }
 ...
 */
async function extractGoogleFormWithCorsProxy(url: string): Promise<Question[]> {
  try {
    ...
    const fbDataMatch = html.match(/FB_PUBLIC_LOAD_DATA_\s*=\s*(.*?);\s*<\/script>/s);
    ...
    try {
      ...
      formItems.forEach((item, index) => {
        ...
        if ((questionType === 'choice' || questionType === 'checkbox') && item[4] && Array.isArray(item[4])) {
 ...
 */
function getGoogleFormQuestionType(googleType: number): "text" | "choice" | "checkbox" | "other" {
  switch (googleType) {
    case 0: // Short answer
    case 1: // Paragraph
      return "text";
...
async function extractAirtableForm(url: string): Promise<Question[]> {
  try {
    ...
    const scriptMatch = html.match(/<script id="formData" type="application\/json">(.*?)<\/script>/s);
    ...
    try {
      ...
      formFields.forEach((field, index) => {
        ...
        if ((questionType === 'choice' || questionType === 'checkbox') && field.options) {
          question.options = field.options.map((opt: any) => opt.name || opt.label || opt.text).filter(Boolean);
        }
...
function extractAirtableFormAlternative(html: string): Question[] {
  try {
    ...
    while ((match = fieldLabelRegex.exec(html)) !== null) {
      ...
      questionText = questionText.replace(/<span class="required">\*<\/span>/, '').trim();
 ...
 */
function getAirtableQuestionType(atType: string): "text" | "choice" | "checkbox" | "other" {
  switch (atType) {
    case "singleLineText":
    case "multilineText":
    case "email":
